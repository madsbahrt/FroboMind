...
class NavigateInRow():
    ...
    def __init__(self,name,rowtopic,replan_heading_threshold,replan_offset_threshold,desired_offset,plan_ahead):
        ...
        self._server = actionlib.SimpleActionServer(self._action_name,navigate_in_rowAction,execute_cb=self.execute_cb)
        self._online = False
        self._subscriber = rospy.Subscriber(rowtopic, claas_row_cam, callback=NavigateInRow.row_msg_callback,callback_args=self)
        self._action_client = actionlib.SimpleActionClient("/follow_path",follow_pathAction)
        ...
        self.__path_msg.poses.append(P1)
        self.__path_msg.poses.append(P2)
        
        g = follow_pathGoal()
        g.path = self.__path_msg;
        
        
        rospy.loginfo("waiting for server")
        
        self._action_client.wait_for_server(rospy.Duration(2));
